...
const Transactions = ({navigation}) => {
  ...
  const [transactions, setTransactions] = React.useState(currentUser?.orders);
  ...
  const renderCallItem = ({ item }) => (
    <View style={styles.callItem}>
      ...
      {item.status}{"    "} {formatDateTime12h(item?.createdAt)}
        </Text>
      ...
      <Text style={styles.callCost}>₹{item.amount}</Text>
    </View>
  );
